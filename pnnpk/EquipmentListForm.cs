...
namespace pnnpk
{
    public partial class EquipmentListForm : Form
    {
        ...
        public EquipmentListForm(int id)
        {
            InitializeComponent();
            itemID = id;

            equipmentQuery += $" where [Оборудование].[ID] <> '{itemID}'";
        ...
        {
            //замена оборудования
            string query = equipmentQuery + $" where [IDОборудования].[ID] = {itemID}";

            dataAdapter.InsertCommand = new SqlCommand(equipmentQuery, MainForm.connection);
            dataAdapter.InsertCommand.ExecuteNonQuery();

            DataRow row = dataTable.Rows[equipment_list.CurrentRow.Index];
